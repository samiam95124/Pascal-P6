...
begin
   ...
   for i := 1 to m do begin putchr(da, s[i]); da := da+1; i := i+1 end

end;

procedure set2atr(view st: settype; out at: attrset)
...
procedure atr2set(view at: attrset; out st: settype)
...
procedure set2prm(view st: settype; out ps: permset)
...
procedure prm2set(view ps: prmset; out st: settype)
...
begin
    ...
    for i := 1 to l do begin putchr(ad, s[i]); ad := ad+1 end
...
procedure cvtflist(fp: filptr; var la: address);

var lp: filptr; ad, ad2, ad3: address;
...
procedure cvtenv(ep: envptr; var la: address);

var lp: envptr; ad, ad2, ad3: address;
...
begin
   ...
   while ep <> nil do begin { translate list }
      ...
      putint(ad2, l); { put string size }
      ad2 := ad2+intsize;
      movstr2vm(ep^.name^, l, ad2); { move the name string into place }
      ...
      putint(ad2, l); { put string size }
      ad2 := ad2+intsize;
      movstr2vm(ep^.data^, l, ad2); { move the name string into place }
      ad3 := ad; { set last entry link }
      putadr(ad, nilval); { clear next (with nil value) }
...
procedure getenv(ad: address; out ep: envptr);
    ...
    case routine of
       ...
       5:  begin { writedate() }
           ...
           if fn <= commandfn then case fn of
              outputfn,
              errorfn,
              listfn: begin writedate(output, a); newline := false end;
              prrfn: writestr(prr, ad1, w, l);
              prdfn,inputfn,
              commandfn: errore(WriteOnReadOnlyFile);
           end else begin
                if filstate[fn] <> fwrite then errore(FileModeIncorrect);
                writedate(filtable[fn], a)
           end
       ...
       15: begin { allenv() }
          ...
          services.allenv(integer(ep)); { get environment list }
       ...
       21: begin { getcur() }
           ...
           services.getcur(s, getint(params+ptrsize));
