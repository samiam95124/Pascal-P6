...
Both }ian types are supported. There is no alignment needed, but you
...
/* type                     #32 #64 */
#define INTSIZE             4   /*8*/  /* size of integer */
#define INTAL               4          /* alignment of integer */
#define INTDIG              11  /*20*/ /* number of decimal digits in integer */
#define INTHEX              8   /*16*/ /* number of hex digits of integer */
...
#define PTRSIZE             4   /*8*/  /* size of pointer */
#define ADRSIZE             4   /*8*/  /* size of address */
...
#define STACKELSIZE         4   /*8*/  /* stack element size */
#define MAXSIZE            32          /* this is the largest type that can be on
                                 the stack */
...
#define MARKSIZE           32   /*56*/ /* maxresult+6*ptrsize */
...
*/
#define MARKFV              -8   /*0*/  /* function value */
#define MARKSL              -12  /*8*/  /* static link */
#define MARKDL              -16  /*16*/ /* dynamic link */
#define MARKEP              -20  /*24*/ /* (old) maximum frame size */
#define MARKSB              -24  /*32*/ /* stack bottom */
#define MARKET              -28  /*40*/ /* current ep */
#define MARKRA              -32  /*48*/ /* return address */
...
typedef int fileno;            /* logical file number */
...
void finish(int e)
{
    for (i = 1; i < MAXFIL; i++) if filstate[i] <> fclosed) fclose(filtable[i]);
    printf("\n");
...

{
    ...
    if (DOCHKDEF) { b = storedef[a div 8]; r := (b / bitmsk[a % 8])&1 }
...
void assignexternal(fileno fn; char hfn[])
{
    int i;
...
int getint(a: address); { chkdef(a); return ((int)store[a]); }
void putint(address a, int x); { (int)store[a] = x; putdef(a, TRUE); }
float getrel(a: address); { chkdef(a); return ((float)store[a]); }
void putrel(address a, float f); { (float)store[a] = r; putdef(a, TRUE); }
boolean getbol(a: address); { chkdef(a); return ((boolean)store[a]); }
void putbol(address a, boolean b); { (float)store[a] = x; putdef(a, TRUE); }
char getchr(a: address); { chkdef(a); return ((char)store[a]); }
void putchr(address a, char c); { (char)store[a] = c; putdef(a, TRUE); }
byte getbyt(a: address); { chkdef(a); return ((byte)store[a]); }
void putbyt(address a, byte b); { (byte)store[a] = c; putdef(a, TRUE); }
address getadr(a: address); { chkdef(a); return ((address)store[a]); }
void putadr(address a, address ad); { (address)store[a] = c; putdef(a, TRUE); }
...

{
    ...
    chkdef(a);
    for (i = 1; i <= setsize; i++) s[i] = (settype *)store[a+i-1];
}
...

{
   ...
   for (i = 1; i <= setsize; i++) (settype*)store[a+i-1] = s[i];
   putdef(a, true);
}
...

{
    ...
    while (c = getc(fp) != EOF && l > 0) {
        ...
        cs := 0;
        for (i = 1; i <= l; i++) {
            fscanf(fp, "%2x", &b); putbyt(ad, b); cs := (cs+b) mod 256;
            ad := ad+1;
        };
...
void valfil(address fa) /* attach file to file entry */
{
    ...
    if (store[fa] == 0) { /* no file */
        if (fa == pctop+inputoff) ff = INPUTFN;
            ...
            ff := 0;
            while (i <= maxfil) {
                if (filstate[i] == fclosed) { ff = i; i = maxfil; }
                i := i+1
            }
...
void valfilwm(address fa) /* validate file write mode */
{
    ...
    if (filstate[store[fa]] != fwrite) errore(FILEMODEINCORRECT)
}

void valfilrm(address fa) /* validate file read mode */
{
    valfil(fa); /* validate file address */
    if (filstate[store[fa]] != fread) errore(FILEMODEINCORRECT);
}
...
void dspspc(address len, address blk)
{
   ...
   if (dorecycl && !dochkrpt) { /* obey recycling requests */
        ...
        if (len >= adrsize*2) putadr(ad+adrsize, abs(getadr(ad))-adrsize);
            { the "marker" is a block with a single address. Since it can't hold
              more than that, it will never be reused }
...
int filelength(FILE* fp)
{
    ...
    s = ftell(filtable[fn]); fseek(filtable[fn], 0, SEEK_END);
    p = ftell(filtable[fn]); fseek(filtable[fn], s, SEEK_SET);

    return (p);
}

boolean eoffns(fileno fn)
{
    boolean eoft;

    if (filstate[fn] == fwrite)
        eoft = ftell(filtable[fn] >= filelength(filtable[fn]);
    else (if filstate[fn] == fread) eoft = eoffile(filtable[fn]);
    else errore(FILENOTOPEN);
}

char buffn(fileno fn)
{
    int c;

    if (fn <= COMMANDFN) select(fn) {
        ...
        case LISTFN:    errore(READONWRITEONLYFILE);
        case COMMANDFN: c = bufcommand;
    } else {
        if (filstate[fn] != fread) errore(FILEMODEINCORRECT);
...
boolean eolnfn(fileno fn)
{
    boolean eoln;

    if (fn <= COMMANDFN) select (fn) {
        case INPUTFN:   eoln = eolnfile(stdin);
        case PRDFN:     eoln = eolnfile(filetable[PRDFN]);
        case PRRFN:     eoln = eolnfile(filetable[PRRFN]);
        case ERRORFN: case outputfn:
        case LISTFN:    errore(FILEMODEINCORRECT);;
        case COMMANDFN: eoln = eolncommand;
    } else {
        if (filstate[fn] == fclosed) errore(FILENOTOPEN);
...
char chkbuf(fileno fn, int w)
{ if (w > 0) return buffn(fn); else return(' '); }

boolean chkend(fileno fn, int w)
{ return (w = 0 || eoffn(fn)); }
end;

void getbuf(fileno fn, int* w)
{
  if (*w > 0) {
...
void readi(fileno fn, int *i, int* w, boolean fld)
{
    int s;
    int integer;
   ...
   while (isdigit(chkbuf(fn, w))) { /* parse digit */
     d = chkbuf(fn, w)-'0';
     if (i > maxint div 10 ||
        (i == maxint div 10 && d > maxint mod 10)
...
void readr(fileno fn, float* r, int w, boolean fld)
{
   ...
   *r = 0.0; /* clear result */
   ...
   while (isdigit(chkbuf(fn, w)) do { /* parse digit */
      d = ord(chkbuf(fn, w))-ord('0');
...
void readc(fileno fn, char* c, int w, boolean fld)
{
...
void reads(fileno fn, address ad, int l, int w, boolean fld)
{
  int c;
  while (l > 0) do {
...
void readsp(fileno fn, address ad,  int l)
{
  while (l > 0 && !eolnfn(fn)) {
    if (eoffn(fn)) errore(ENDOFFILE);
    read(filtable[fn], c); putchr(ad, c); ad = ad+1; l = l-1
  }
  while (l > 0) do { putchr(ad, ' '); ad = ad+1; l = l-1 }
}
...
void resetfn(fileno fn, boolean bin)
{
    /* file was closed, no assigned name, give it a temp name */
    if (filstate[fn] == fclose && !filanamtab[fn]) tmpnam(filnamtab[fn]);
    filstate[fn] = fread;
    if (!fopen(filnamtab[fn], bin?"rb":"r")) errore(FILEOPENFAIL);
    filbuff[fn] = FALSE;
}

void rewritefn(fileno fn, boolean bin)
{
    /* file was closed, no assigned name, give it a temp name */
    if (filstate[fn] == fclose && !filanamtab[fn]) tmpnam(filnamtab[fn]);
    filstate[fn] = fwrite;
...

{
    ...
    case 1 /*put*/: popadr(ad); valfil(ad); fn = store[ad];
                    if (fn <= COMMANDFN) select (fn) {
                       ...
                       case COMMANDFN: errore(WRITEONREADONLYFILE); break;
                         ...
                         if (filstate[fn] != fwrite) errore(FILEMODEINCORRECT);
                         putfile(filtable[fn], ad, fn);
                    }
                    break;
    case 3 /*rln*/: popadr(ad); pshadr(ad); valfil(ad); fn = store[ad];
                    if (fn <= COMMANDFN) select (fn) {
                       INPUTFN: readline(stdin); break;
                       PRDFN: readline(filtable[PRDFN]); break;
                       OUTPUTFN: prrfn: errorfn:
                       LISTFN: errore(READONWRITEONLYFILE); break;
                       COMMANDFN: readlncommand; break;
                    } else {
                         if (filstate[fn] != fread) errore(FILEMODEINCORRECT);
    ...
    case 65 /*wrsp*/: popint(l); popadr(ad1); popadr(ad); pshadr(ad);
                    ...
                    if (w < 1 && ISO7185) errore(INVALIDFIELDSPECIFICATION);
                    if (fn <= COMMANDFN) select (fn) {
                      case OUTPUTFN: writestrp(output, ad1, l); break;
                      case PRRFN: writestrp(filtable[PRRFN], ad1, l); break;
                      case ERRORFN: writestrp(output, ad1, l); break;
                      case LISTFN: writestrp(output, ad1, l); break;
                      case PRDFN: case INPUTFN:
                      case COMMANDFN: errore(WriteOnReadOnlyFile); break;
                    } else {
                      if (filstate[fn] != fwrite) errore(FILEMODEINCORRECT);
                    ...
                    break;
    case 1 /*eof*/: popadr(ad); valfil(ad); fn := store[ad];
                    pshint(eoffn(fn));
                    break;
    case 2 /*efb*/:
                 popadr(ad); valfilrm(ad); fn = store[ad];
                 if (filstate[fn] == fwrite) pshint(TRUE);
                 else if (filstate[fn] == fread)
    ...
    case 69 /*wizb*/: { popint(w); popint(i); popadr(ad); pshadr(ad);
                     ...
                     if (w < 1 && ISO7185) errore(INVALIDFIELDSPECIFICATION);
                     if (fn <= COMMANDFN) select (fn) {
                          case OUTPUTFN: writei(output, i, w, rd, lz);
                          case PRRFN: writei(filtable[PRRFN], i, w, rd, lz);
                          case ERRORFN: writei(output, i, w, rd, lz);
                          case LISTFN: writei(output, i, w, rd, lz);
                          case PRDFN: case INPUTFN:
    ...
    case 9 /*wrr*/: popint(w); poprel(r); popadr(ad); pshadr(ad);
                     ...
                     if (fn <= COMMANDFN) select (fn) {
                          OUTPUTFN: fprintf(stdout, "%*.*e", w, w-REALEF, r); break;
                          PRRFN: fprintf(filtable[PRRFN], "%*.*e", w, w-REALEF, r); break;
                          ERRORFN: fprintf(stderr, "%*.*e", w, w-REALEF, r); break;
                          LISTFN: fprintf(stdout, "%*.*e", w, w-REALEF, r); break;
                          PRDFN: INPUTFN:
    ...
    case 17/*log*/: poprel(r1); if (r1 <= 0) errore(INVALIDARGUMENTTOLN);
                    pshrel(log(r1)); break;
    case 18/*sqt*/: poprel(r1); if (r1 < 0) then errore(INVALIDARGUMENTTOSQRT);
    ...
    case 21/*pag*/: popadr(ad); valfil(ad); fn := store[ad];
                    if (fn <= COMMANDFN) then case fn of
                         OUTPUTFN: fprintf(stdout, "\f"); break;
                         PRRFN: fprintf(filtable[PRRFN], "\f"); break;
                         ERRORFN: fprintf(stderr, "\f"); break;
                         LISTFN: fprintf(stdout, "\f"); break;
                         PRDFN: INPUTFN:
                         COMMANDFN: errore(WRITEONREADONLYFILE); break;
                    } else {
                         if (filstate[fn] != fwrite) errore(FILEMODEINCORRECT);
    ...
    case 22/*rsf*/: popadr(ad); valfil(ad); fn = store[ad];
                    if (fn <= COMMANDFN) select (fn) {
                         PRDFN: resetfn(PRDFN, FALSE); break;
                         PRRFN: errore(CannotResetWriteOnlyFile); break;
    ...
    case 23/*rwf*/: popadr(ad); valfil(ad); fn = store[ad];
                    if (fn <= COMMANDFN) select (fn) of
                         PRRFN: rewritefn(PRRFN, FALSE); break;
                         PRDFN: errore(CANNOTREWRITEREADONLYFILE); break;
                         ERRORFN: LISTFN: OUTPUTFN: INPUTFN:
                         COMMANDFN:
                           errore(CANNOTRESETORREWRITESTANDARDFILE); break;
                    } else rewritefn(filtable[fn], FALSE);
                    break;
    ...
    case 33/*rsb*/: popadr(ad); valfil(ad); fn = store[ad]; resetfn(fn, TRUE);
                    break;
    case 34/*rwb*/: popadr(ad); valfil(ad); fn = store[ad]; rewrite(fn, TRUE);
                    break;
    ...
    case 44 /*fvb*/: popint(i); popadr(ad); pshadr(ad); valfil(ad);
                   ...
                   /* load buffer only if in read mode, and buffer is
                     empty */
                   if (filstate[fn] = fread && !filbuff[fn]) {
                       for (j = 0; j < i; j++) {
    ...
    case 49 /*upd*/: popadr(ad); valfil(ad); fn := store[ad];
                if (filstate[fn] == fread) seek(filtable[fn], 0, SEEK_SET);
                else {
                  if (fclose(filtable[fn])) errorv(FILECLOSEFAIL);
    ...
    case 50 /*appt*/: popadr(ad); valfil(ad); fn := store[ad];
                if (filstate[fn] == fwrite) seek(filtable[fn], 0, SEEK_END);
                else {
                  if (fclose(filtable[fn])) errorv(FILECLOSEFAIL);
    ...
    case 79/*aefb*/: popint(i); popadr(ad1); popadr(ad); valfil(ad);
                  fn = store[ad]; clrfn(fl1);
                  for (j = 0; j < i; j++); do fl1[j] = store[ad1+j];
                  assignexternal(fn, fl1)
                };
...

{
    ...
    case 68  /*ldob*/: getq(); pshint(ord(getbol(q))); break;
    case 69  /*ldoc*/: getq(); pshint(ord(getchr(q))); break;
    ...
    case 79  /*sroc*/: getq(); popint(i1); c1 = chr(i1); putchr(q, c1); break;
    ...
    case 139 /* equb */:
    case 141 /* equc */:
    case 137 /* equi */: popint(i2); popint(i1); pshint(ord(i1=i2)); break;
    case 138 /* equr */: poprel(r2); poprel(r1); pshint(ord(r1=r2)); break;
    case 140 /* equs */: popset(s2); popset(s1); pshint(ord(s1=s2)); break;
    case 142 /* equm */: getq(); popadr(a2); popadr(a1); compare(b, a1, a2);
                         pshint(ord(b)); break;

    case 18  /* neqa */: popadr(a2); popadr(a1); pshint(ord(a1<>a2)); break;
    case 145 /* neqb */:
    case 147 /* neqc */:
    case 143 /* neqi */: popint(i2); popint(i1); pshint(ord(i1<>i2)); break;
    case 144 /* neqr */: poprel(r2); poprel(r1); pshint(ord(r1<>r2)); break;
    case 146 /* neqs */: popset(s2); popset(s1); pshint(ord(s1<>s2)); break;
    case 148 /* neqm */: getq(); popadr(a2); popadr(a1); compare(b, a1, a2);
                         pshint(!b)); break;

    case 151 /* geqb */:
    case 153 /* geqc */:
    case 149 /* geqi */: popint(i2); popint(i1); pshint(ord(i1>=i2)); break;
    case 150 /* geqr */: poprel(r2); poprel(r1); pshint(ord(r1>=r2)); break;
    case 152 /* geqs */: popset(s2); popset(s1); pshint(ord(s1>=s2)); break;
    case 154 /* geqm */: getq(); popadr(a2); popadr(a1); compare(b, a1, a2);
                         pshint(ord(b or (store[a1] >= store[a2]))); break;

    case 157 /* grtb */:
    case 159 /* grtc */:
    case 155 /* grti */: popint(i2); popint(i1); pshint(ord(i1>i2)); break;
    case 156 /* grtr */: poprel(r2); poprel(r1); pshint(ord(r1>r2)); break;
    case 158 /* grts */: errorv(SETINCLUSION); break;
    case 160 /* grtm */: getq(); popadr(a2); popadr(a1); compare(b, a1, a2);
                         pshint(ord(not b and (store[a1] > store[a2]))); break;

    case 163 /* leqb */:
    case 165 /* leqc */:
    case 161 /* leqi */: popint(i2); popint(i1); pshint(ord(i1<=i2)); break;
    case 162 /* leqr */: poprel(r2); poprel(r1); pshint(ord(r1<=r2)); break;
    case 164 /* leqs */: popset(s2); popset(s1); pshint(ord(s1<=s2)); break;
    case 166 /* leqm */: getq(); popadr(a2); popadr(a1); compare(b, a1, a2);
                         pshint(ord(b or (store[a1] <= store[a2]))); break;

    case 169 /* lesb */:
    case 171 /* lesc */:
    case 167 /* lesi */: popint(i2); popint(i1); pshint(ord(i1<i2)); break;
    case 168 /* lesr */: poprel(r2); poprel(r1); pshint(ord(r1<r2)); break;
    case 170 /* less */: errorv(SETINCLUSION); break;
    case 172 /* lesm */: getq(); popadr(a2); popadr(a1); compare(b, a1, a2);
                         pshint(ord(not b and (store[a1] < store[a2]))); break;

    case 23 /*ujp*/: getq(); pc := q; break;
    case 24 /*fjp*/: getq(); popint(i); if i = 0 then pc := q; break;
    ...
    case 35 /*trc*/: poprel(r1);
                  if (DOCHKOVF) if ((r1 < -maxint) or (r1 > maxint) then
    ...
    case 53 /*dvi*/: popint(i2); popint(i1); break;
                      ...
                      pshint(i1 div i2); break;
    case 54 /*dvr*/: poprel(r2); poprel(r1);
                      if (DOCHKOVF) if (r2 = 0.0) then errore(ZERODIVIDE);
                      pshrel(r1/r2); break;
    case 55 /*mov*/: getq(); popint(i2); popint(i1);
                 for (i3 = 0; i3 <= q-1; i3++) do
    ...
    case 62 /*rnd*/: poprel(r1);
                  if (DOCHKOVF) if (r1 < -(INT_MAX+0.5)) || r1 > maxint+0.5)
                    errore(REALARGUMENTTOOLARGE);
