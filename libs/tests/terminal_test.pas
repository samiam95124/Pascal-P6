...
var
   ...
   i, j, b, tc, cnt, tn:         integer;
   clk:                          integer;
   tf:                           text; { test file }
   eventflag1, eventflag2:       boolean;
   oeh1, oeh2:                   integer; { old event handlers }
   line:                         packed array [1..250] of char;
   ln, sn, etn, esn:             integer; { lock, signal and thread numbers }
   timeout1, timeout2:           integer; { timer signals }
   thdstp, ethdstp:              boolean; { thread stop flags }
   bi:                           bench;
...
begin
   ...
   repeat
      ...
      if er.etype = ettim then begin
         ...
         if er.timnum = 1 then sendsig(timeout1)
         else if er.timnum = 2 then sendsig(timeout2)
   ...
   until stop;
   lock(ln);
   sendsig(esn); { signal thread complete }
   unlock(ln)

end;

{ draw an expanding/contracting box on a separate thread }
...
begin
   ...
   lock(ln);
   sendsig(sn); { signal thread complete }
   unlock(ln)
...
begin
   ...
   curvis(output, false);
   page;
   writeln('The left and right figures are run on different threads');
   prtcen(maxy(output), 'Threading test');
   thdstp := false;
   ethdstp := false;
   ln := initlock;
   sn := initsig;
   ...
   timeout2 := initsig;
   tn := newthread(thread);
   etn := newthread(eventthread);
   timer(output, 1, second div 10, true);
   timer(output, 2, second div 10, true);
   x := maxx(output) div 3;
   y := maxy(output) div 2;
   for j := 1 to 30 do
      for i := 0 to 9 do begin
   ...
   { stop subthread }
   lock(ln); thdstp := true; waitsig(ln, sn); unlock(ln);
   { stop event thread }
   lock(ln); ethdstp := true; waitsig(ln, esn); unlock(ln);
   killtimer(output, 1);
   killtimer(output, 2);
   cursor(output, 1, 3);
   deinitlock(ln);
