#!/usr/bin/env python3
# Pascal emitter: generate the per-module external executors for the
# interpreter's flavored extexec modules (source/{plain,term,graph}/extexec.pas),
# used by the Pascal interpreters pint/pmach.
#
# The spec parsing here (parse_interface, toks, parse_evtrec, and the Gen class's
# spec loading) is reused by the C emitter, tools/extgen/gencexec.py, which
# emits the equivalent marshalling in C for the cmach interpreter. Keep this
# importable: main() runs only under __main__ so importing does not regenerate.
#
...
class Gen:
    ...
    def __init__(self, module):
        ...
        # when set, generate terminal file params the graphics way (filtable[fn],
        # the hosted window file) rather than the interpreter console input/output
        # -- used for the terminal executor hosted under the graphics flavor.
        self.graphfiles = False
...
# The plain flavor stubs both terminal and graphics, so it needs only the error
# code the stubs raise (the full HELPERS marshalling set would be dead code).
PLAIN_DECL = '''const FunctionNotImplemented = 90; { external function not implemented }
'''

def fwd(name):
    """forward declaration of an executor entry point"""
    return 'procedure %s(routine: integer; var params: integer); forward;' % name

# the two entry points every extexec flavor declares (real or stubbed)
EXEC_FWD = fwd('execterminal') + '\n' + fwd('execgraph')
...
def writemodule(path, header, name, joins, usesmods, decls, body):
    """Emit a Pascaline module: the header comment, the module name, an optional
       joins clause, an optional uses clause, the public declarations, then the
       private body."""
    out = [header, '', 'module %s;' % name, '']
    if joins:
        out += ['joins %s;' % joins, '']
    if usesmods:
        out += ['uses %s;' % usesmods, '']
...
def main():
    ...
    # Sound and network are standard in every flavor: their executors are
    # identical across plain, terminal and graphics, so rather than copy them
    # into each per-flavor extexec they live once in source/common/execcommon.
    # source/common is on the module path of every flavor (added to the pint and
    # pmach instruction files), so all three interpreters link the single copy.
    # The per-flavor extexec then carries only the terminal/graphics executors --
    # real where the flavor hosts the model, a one-line stub otherwise.
    #
    # The marshalling helpers (HELPERS) stay private to each executor module: a
    # module's public names are re-exported up the uses chain to extlink and on
    # to pint/pmach, which carry their own getstr/str/FunctionNotImplemented, so
    # a shared public helper module would collide. Kept private they do not leak;
    # the small duplication is the price of not polluting the interpreter scope.

    common = os.path.join(ROOT, 'source/common')
    os.makedirs(common, exist_ok=True)

    # sound and network executors, standard in every flavor
    cbody = [HELPERS]
    addmods(cbody, [sound, network])
    writemodule(os.path.join(common, 'execcommon.pas'),
'''{*******************************************************************************
*                                                                              *
...
*                                                                              *
*******************************************************************************}''',
        'execcommon', 'sound, network', 'pint_mem',
        fwd('execsound') + '\n' + fwd('execnetwork'),
        '\n'.join(cbody))

    # plain flavor: terminal and graphics both stubbed
    writemodule(os.path.join(ROOT, 'source/plain/extexec.pas'),
'''{*******************************************************************************
*                                                                              *
...
*                                                                              *
*******************************************************************************}''',
        'extexec', None, 'pint_mem', EXEC_FWD,
        '\n'.join([PLAIN_DECL, STUB % 'execterminal', STUB % 'execgraph']))

    # terminal flavor: real terminal executor, graphics stubbed
    tbody = [HELPERS]
    ...
    writemodule(os.path.join(ROOT, 'source/term/extexec.pas'),
'''{*******************************************************************************
*                                                                              *
*                       External model execution                               *
*                                                                              *
...
*                                                                              *
*******************************************************************************}''',
        'extexec', 'terminal', 'pint_mem', EXEC_FWD,
        '\n'.join(tbody))

    # graphics flavor: graphics is 100% upward-compatible with terminal, so this
    # flavor hosts BOTH. The graphics archive provides the terminal functions too
    # (terminal.a cannot be co-linked -- they share the ami_* core), so the
    # terminal executor is emitted graphics-qualified (terminal.X -> graphics.X)
    # and reuses the graphics converters and event marshalling emitted above
    # (every helper the terminal executor needs is a subset of the graphics set).
    ...
    # terminal executor hosted under graphics: graphics-qualified calls, and file
    # params routed to the hosted window files (filtable[fn]) like the graphics
    # executor, not the interpreter console.
    term_g = Gen('terminal')
    term_g.graphfiles = True
    gbody += [ln.replace('terminal.', 'graphics.')
              for ln in term_g.executor('execterminal')]
    writemodule(os.path.join(ROOT, 'source/graph/extexec.pas'),
'''{*******************************************************************************
*                                                                              *
*                       External model execution                               *
*                                                                              *
...
*                                                                              *
*******************************************************************************}''',
        'extexec', 'graphics', 'pint_mem', EXEC_FWD,
        '\n'.join(gbody))
